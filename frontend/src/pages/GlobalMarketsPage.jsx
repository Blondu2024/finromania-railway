            ...
            <div className="mt-4 flex items-end gap-6">
              <div>
                <p className="text-4xl font-bold">
                  ...
                  {asset.unit && <span className="text-lg ml-2 opacity-80">{asset.unit}</span>}
                </p>
              </div>
            ...
            <div className="h-80 w-full">
              {loading ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    ...
                    <YAxis 
                      ...
                      tick={{ fontSize: 11 }}
                      tickFormatter={(value) => value.toLocaleString('ro-RO', { maximumFractionDigits: 0 })}
                    />
                    <Tooltip
                      contentStyle={{ 
                      ...
                      formatter={(value) => [value?.toLocaleString('ro-RO', { maximumFractionDigits: 2 }), 'Preț']}
                      labelFormatter={(label) => `📅 ${label}`}
                    />
                    <ReferenceLine y={asset.prev_close} stroke="#6b7280" strokeDasharray="5 5" />
                    <Area
                      type="monotone"
                      dataKey="close"
                ...
                <div className="h-full flex items-center justify-center text-muted-foreground">
                  Nu sunt date disponibile pentru acest activ
                </div>
              )}
            </div>
            ...
            {chartData.length > 0 && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
                <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-3">
                  <p className="text-xs text-muted-foreground">Maxim Perioadă</p>
                  <p className="text-lg font-bold text-green-600">
                    {Math.max(...chartData.map(d => d.high || d.close)).toLocaleString('ro-RO', { maximumFractionDigits: 2 })}
                  </p>
                </div>
                <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-3">
                  <p className="text-xs text-muted-foreground">Minim Perioadă</p>
                  <p className="text-lg font-bold text-red-600">
                    {Math.min(...chartData.map(d => d.low || d.close)).toLocaleString('ro-RO', { maximumFractionDigits: 2 })}
                  </p>
                </div>
                <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-3">
                  <p className="text-xs text-muted-foreground">Închidere Anterioară</p>
                  <p className="text-lg font-bold">{asset.prev_close?.toLocaleString('ro-RO', { maximumFractionDigits: 2 })}</p>
                </div>
                <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-3">
                  <p className="text-xs text-muted-foreground">Variație Perioadă</p>
                  <p className={`text-lg font-bold ${chartInfo?.period_change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {chartInfo?.period_change >= 0 ? '+' : ''}{chartInfo?.period_change?.toFixed(2)}%
                  </p>
                </div>
              </div>
            )}
